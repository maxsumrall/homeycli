      ...
      throw cliError(
        ...
        'local mode selected but local Homey address/token are not configured',
        {
          ...
          help:
            ...
            '     export HOMEY_ADDRESS="http://<homey-ip>"\n' +
            '     echo "<LOCAL_API_KEY>" | homeycli auth set-local --address "$HOMEY_ADDRESS" --stdin\n\n' +
...
async function authSetLocal(token, options) {
  ...
  if (!address) {
    throw cliError('INVALID_VALUE', 'address is required (use --address http://<homey-ip>)');
  }
